...
class BackgroundLocationService: MethodChannel.MethodCallHandler, PluginRegistry.RequestPermissionsResultListener {
    ...
    fun setActivity(binding: ActivityPluginBinding?) {
        ...
        if(this.activity != null){
            ...
            if (appContext != null && Utils.requestingLocationUpdates(appContext)) {
                if (!checkPermissions()) {
                    requestPermissions()
                }
            }
    ...
    private fun stopLocationService(): Int {
        ...
        context?.let { appContext ->
            unregisterReceiverIfNeeded(appContext)
    ...
    private fun checkPermissions(): Boolean {
        val appContext = context ?: return false
        return PackageManager.PERMISSION_GRANTED == ActivityCompat.checkSelfPermission(appContext, Manifest.permission.ACCESS_FINE_LOCATION)
    }
...
override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray): Boolean{
    ...
    private fun unregisterReceiverIfNeeded(appContext: Context) {
        if (isReceiverRegistered) {
            receiver?.let {
                try {
                    LocalBroadcastManager.getInstance(appContext).unregisterReceiver(it)
                } catch (ignored: IllegalArgumentException) {
                    // Keep detach/shutdown resilient even if receiver state gets out of sync.
                } finally {
